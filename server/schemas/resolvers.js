// Importing the User model and utilities related to authentication
const { User } = require('../models');
const { signToken, AuthenticationError } = require('../utils/auth');

// GraphQL resolvers defining operations for queries and mutations
const resolvers = {
  Query: {
    // Resolver handling the 'me' query, retrieving a user based on their ID
  ...
  Mutation: {
    // Resolver handling the 'addUser' mutation, creating a new user and generating a token
    addUser: async (parent, { username, email, password }) => {
      ...
      // Return the generated token along with the user
      return { token, user };
    },

    // Resolver handling the 'login' mutation, managing user login and token generation
    ...
    // Resolver handling the 'saveBook' mutation, adding a book to a user's saved books
    ...
    // Resolver handling the 'removeBook' mutation, removing a book from a user's saved books
